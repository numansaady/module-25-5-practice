...
const h1items = document.getElementsByTagName("h1");
for (const h1item of h1items) {
  h1item.style.color = "lightblue";
}
...
backPack.style.padding = "50px 20px";
backPack.style.borderRadius = "20px";
...
const buyBtns = document.getElementsByClassName("buy-now-button");
...
const subsField = document.getElementById("subscribe-field");
const subsBtn = document.getElementById("subscribe-btn");
subsField.addEventListener("keyup", function (event) {
  if (event.target.value == "email") {
    subsBtn.removeAttribute("disabled");
  } else {
    subsBtn.setAttribute("disabled", true);
  }
});
